...
const AdvisorAssignment: React.FC = () => {
    ...
    const handleResetAll = async () => {
        if (!confirm('PERHATIAN: Ini akan menghapus SEMUA penugasan pembimbing. Lanjutkan?')) return;

        setResetting(true);
        setShowResetMenu(false);
        try {
            const response = await api.post('/advisor-assignment/reset-all');
    ...
    // Reset specific advisor assignments
    const handleResetAdvisor = async (advisorId: string, advisorName: string) => {
        if (!confirm(`Hapus semua siswa dari pembimbing ${advisorName}?`)) return;

        setResetting(true);
        setShowResetMenu(false);
        try {
            const response = await api.post(`/advisor-assignment/reset-advisor/${advisorId}`);
            setNotification({ type: 'success', message: response.data.message });
            setCanUndo(response.data.canUndo);
            await fetchData();
        } catch (error: any) {
            setNotification({ type: 'error', message: error.response?.data?.error || 'Gagal reset pembimbing' });
        } finally {
            setResetting(false);
        }
    };
    ...
    const handleUndo = async () => {
        ...
        setShowResetMenu(false);
        try {
            const response = await api.post('/advisor-assignment/undo');
            setNotification({ type: 'success', message: response.data.message });
            setCanUndo(false);
            await fetchData();
        } catch (error: any) {
                                ...
                                <div className="relative">
                                    ...
                                    {showResetMenu && (
                                        <div className="absolute right-0 mt-1 w-56 bg-white rounded-lg shadow-xl border border-gray-200 z-10 overflow-hidden">
                                            ...
                                            {advisorCapacity && advisorCapacity.advisors.length > 0 && (
                                                <>
                                                    <div className="p-2 border-t border-gray-100">
                                                        <p className="text-xs font-medium text-gray-500 px-2">Reset per Pembimbing</p>
                                                    </div>
                                                    <div className="p-1 max-h-40 overflow-y-auto">
                                                        {advisorCapacity.advisors.filter(a => a.currentStudents > 0).map(advisor => (
                                                            <button
                                                                key={advisor.id}
                                                                onClick={() => handleResetAdvisor(advisor.id, advisor.name)}
                                                            ...
                                                            >
                                                                ...
                                                                <span className="text-xs text-gray-400">{advisor.currentStudents} siswa</span>
                                                            </button>
                                                        ))}
                                                    </div>
                                                </>
